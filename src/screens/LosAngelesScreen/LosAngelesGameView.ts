...
export class LAMapView {
 ...
 constructor(screenSwitcher: ScreenSwitcher) {
   ...
   const existingStage = Konva.stages[0];
 }
 ...
 private drawRegions(): void {
   const regionDefs = [
     { fill: "rgba(0,8,255,0.15)", points: [250,150,400,150,400,250,300,250,300,400,250,400] },
     { fill: "rgba(42,221,39,0.15)", points: [300,250,400,250,400,620,250,620,250,400,300,400] },
     { fill: "rgba(255,0,0,0.1)", points: [400,150,600,150,600,400,400,400] },
     { fill: "rgba(255,174,0,0.1)", points: [600,100,900,100,900,400,600,400] },
     { fill: "rgba(0,200,255,0.15)", points: [400,400,700,400,700,620,400,620] },
     { fill: "rgba(95,69,20,0.19)", points: [250,20,500,20,500,150,250,150] },
   ];
 ...
 private writeStatus(): void {
   const cities = [
     "Burbank","Hollywood","Inglewood","LAX Airport","Long Beach",
     "Monterey Park","Pasadena","Santa Fe Springs","Santa Monica","Union Station"
   ];
   ...
   this.group.add(new Konva.Text({ x: 0, y: offset, text: "Cities", fontSize: 16, fill: "#444", fontStyle: "bold" }));
   this.group.add(new Konva.Text({ x: shift - 50, y: offset, text: "Explored", fontSize: 16, fill: "#444", fontStyle: "bold" }));


   cities.forEach((c, i) => {
     const status = isCityCompleted(c);
     const result = status ? "✅" : "❌";
     this.group.add(new Konva.Text({ x: 0, y: offset + 40 * (i + 1), text: c, fontSize: 16, fill: "#444" }));
     this.group.add(new Konva.Text({ x: shift, y: offset + 40 * (i + 1), text: result, fontSize: 16, fill: "#444" }));
   });
 }
 ...
 private drawFinishBtn(): void {
   ...
   const cities = ["Burbank","Hollywood","Inglewood",
     "LAX Airport","Long Beach","Monterey Park",
...
function handleCityClick(city: string, layer: Konva.Layer): void {
 ...
 if (cityProgress[city] === false){
   // group.visible(false);
   switch (city) {
   case "Burbank": startBBGame(layer); break;
   case "Hollywood": startHWGame(layer); break;
   case "Inglewood": startIGGame(layer); break;
   case "LAX Airport": startLAXGame(layer); break;
   case "Long Beach": startLBGame(layer); break;
   case "Monterey Park": startMPGame(layer); break;
   case "Pasadena": startPDGame(layer); break;
   case "Santa Fe Springs": startSFSGame(layer); break;
   case "Santa Monica": startSMGame(layer); break;
   case "Union Station": startUSGame(layer); break;
   default: alert(`No game found for ${city}`);
 }
 } else {
